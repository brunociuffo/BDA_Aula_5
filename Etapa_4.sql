...
--
DELIMITER $$
CREATE TRIGGER `atualizaQtdeVendida` AFTER INSERT ON `vendas` FOR EACH ROW BEGIN
    UPDATE cardapio SET qtdeVendida = qtdeVendida + NEW.quantidade WHERE codigoProduto = NEW.codigoProduto;
END
$$
DELIMITER ;
-- --------------------------------------------------------
